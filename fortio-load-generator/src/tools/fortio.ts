import { z } from 'zod';
import { FortioRunner, FortioLoadTestParams } from '../utils/fortio.js';
...
export const runLoadTest = {
  ...
  handler: async (params: z.infer<typeof runLoadTestSchema>) => {
    try {
      ...
      const loadTestParams: FortioLoadTestParams = {
        url: params.url,
...
export const createLoadProfile = {
  ...
  handler: async (params: z.infer<typeof createLoadProfileSchema>) => {
    try {
...
export const parseTestResults = {
  ...
  handler: async (params: z.infer<typeof parseTestResultsSchema>) => {
    try {
      ...
      for (const line of lines) {
        if (line.includes('qps')) {
          ...
          if (qpsMatch) metrics.qps = parseFloat(qpsMatch[1]);
        }
        if (line.includes('avg')) {
          const avgMatch = line.match(/avg ([\d.]+)/);
          if (avgMatch) metrics.averageLatency = parseFloat(avgMatch[1]);
        }
        if (line.includes('calls')) {
          const callsMatch = line.match(/(\d+) calls/);
          if (callsMatch) metrics.totalCalls = parseInt(callsMatch[1]);
        }
      }
...
// Helper functions
function generateLoadProfile(params: z.infer<typeof createLoadProfileSchema>) {
  const steps = [];
...
function getProfileDescription(profileType: string): string {
  switch (profileType) {
    case 'constant':
...
function calculateStepDuration(totalDuration: string, steps: number): string {
...
function calculateTotalDuration(profile: any[]): string {
  return `${profile.length} steps - varies by profile type`;
}

function generateSummaryAnalysis(metrics: any): string {
  if (!metrics.qps) return 'Unable to analyze - insufficient metrics';
