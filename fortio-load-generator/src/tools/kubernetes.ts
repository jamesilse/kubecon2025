...
export const listServices = {
  ...
  handler: async (params: z.infer<typeof listServicesSchema>) => {
    try {
      ...
      return {
        ...
        services: services.map((service: any) => ({
          ...
          ports: service.ports.map((port: any) => `${port.port}/${port.protocol}${port.name ? ` (${port.name})` : ''}`),
...
export const getServiceEndpoints = {
  ...
  handler: async (params: z.infer<typeof getServiceEndpointsSchema>) => {
    try {
      ...
      return {
        ...
        endpoints: endpoints.endpoints.map((ep: any) => ({
          ip: ep.ip,
          hostname: ep.hostname,
          ports: ep.ports
        })),
        urls: endpoints.servicePorts.map((port: any) => ({
          port: port.port,
...
export const checkServiceHealth = {
  ...
  handler: async (params: z.infer<typeof checkServiceHealthSchema>) => {
    try {
