...
export class KubernetesClient {
    ...
    async getServiceEndpoints(serviceName, namespace = 'default') {
        try {
            ...
            const endpointAddresses = endpoints.subsets?.flatMap((subset) => subset.addresses?.map((addr) => ({
                ip: addr.ip || '',
    ...
    async checkServiceHealth(serviceName, namespace = 'default') {
        try {
    ...
    buildServiceUrl(serviceName, namespace, port, protocol = 'http') {
